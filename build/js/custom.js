...
(function(root, factory) {
  ...
  var Numbered = function(target, params) {
    ...
    params =
      params ||
      (typeof self.inputs[0].numbered !== "undefined"
        ? self.inputs[0].numbered.params
        : {});
    ...
    self.config.placeholder = self.params.mask.replace(
      new RegExp(self.params.numbered, "g"),
      self.params.empty
    );
    self.config.numbered = self.params.numbered.replace(
      /([()[\]\.^\#$|?+-])/g,
      "\\\\$1"
    );
    self.config.numberedCol =
      self.params.mask.split(self.params.numbered).length - 1;
    self.config.empty = self.params.empty.replace(
      /([()[\]\.^\#$|?+-])/g,
      "\\$1"
    );
    self.config.mask = self.params.mask
      .replace(/([()[\]\.^\#$|?+-])/g, "\\$1")
      .replace(new RegExp(self.config.numbered, "g"), "(\\d)");
    ...
    self.magic = function(event) {
      ...
      for (var key in maskSplit) {
        ...
        key = parseInt(key);
        if (
          maskNumsIndex <= numbered.config.maskNumsCol &&
          val == numbered.config.maskNums[maskNumsIndex] &&
      ...
      if (event.type !== "click") {
        if (
          ...
          valueFormattedIndex - maskNumsIndex === 0 &&
          !numbered.params.placeholder
        ) {
          this.value = "";
        } else if (
          numbered.oldValue !== numbered.input.value ||
          event.type === "focusin"
        ) {
          this.value = value;
        }
      ...
      if (
        ...
        event.type !== "blur" &&
        (event.type !== "click" ||
          (numbered.lastEvent === "focusin" && event.type === "click"))
      ) {
    ...
    for (var index in self.inputs) {
      ...
      var is = false;
      if (
        typeof $input.numbered === "оbject" ||
        typeof $input.numbered !== "undefined"
      ) {
        is = true;
      }
      $input.numbered = {
    ...
    self.validate = function(i) {
      ...
      for (var index in inputs) {
        ...
        if (
          inputs[index].numbered.config.regexp.test(
            inputs[index].numbered.input.value
          )
        ) {
          validate = 1;
        } else if (
          inputs[index].numbered.input.value === "" ||
          inputs[index].numbered.input.value ===
            inputs[index].numbered.config.placeholder
        ) {
          validate = 0;
        } else {
          validate = -1;
